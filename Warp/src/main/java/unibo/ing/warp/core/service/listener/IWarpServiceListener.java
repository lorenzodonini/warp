 ...
 * from a service that was just called. As for every listener, it is possible to either implement
...
public interface IWarpServiceListener {
    /**
     * This is the only method that needs to be implemented by the listener. Its' purpose is to
     * asynchronously perform a callback action on the calling object, once the service is
     * completed. After the listener is triggered, it is possible to retrieve the result
